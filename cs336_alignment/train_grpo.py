...
def main():
    ...
    for step in range(1,args.grpo_steps+1):
        ...
        policy.train(); updates=0; clip_sum=0.0; loss_sum=0.0; entropy_sum=0.0; entropy_count=0; grad_norm_sum=0.0
        for ep in range(args.epochs_per_rollout):
            ...
            for pos,j in enumerate(order):
                ...
                loss,meta=grpo_microbatch_train_step(lp['log_probs'],mask,args.grad_accum,lt,rr,aa,olp,args.cliprange,args.length_normalization,args.constant_normalizer)
                loss_sum += float(loss.detach())*args.grad_accum; clip_sum += float(meta.get('clip_fraction',torch.tensor(0.0))); entropy_sum += float((lp['token_entropy'] * mask).sum().detach()); entropy_count += int(mask.sum()); acc+=1
