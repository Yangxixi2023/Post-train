...
def main():
    ...
    ap.add_argument('--epochs-per-rollout',type=int,default=1); ap.add_argument('--loss-type',choices=['no_baseline','reinforce_with_baseline','grpo_clip'],default='grpo_clip')
    ...
    ap.add_argument('--lr',type=float,default=1e-5); ap.add_argument('--grad-accum',type=int,default=8); ap.add_argument('--max-seq-len',type=int,default=768)
    ...
    def eval_now(step):
        ...
        m,_=evaluate_vllm(llm,r1_zero_reward_fn,valp,valg,temperature=1.0,max_tokens=args.max_new_tokens,seed=args.seed+1000+step,output_path=str(out/f'eval_grpo{step}.jsonl'))
    ...
    for step in range(1,args.grpo_steps+1):
        ...
        adv,raw,rmeta=compute_group_normalized_rewards(r1_zero_reward_fn,flatr,flatg,args.group_size,1e-6,args.normalize_by_std)
        ...
        policy.train(); updates=0; clip_sum=0.0; loss_sum=0.0
        for ep in range(args.epochs_per_rollout):
            ...
            for pos,j in enumerate(order):
                ...
                loss,meta=grpo_microbatch_train_step(lp['log_probs'],mask,args.grad_accum,lt,rr,aa,olp,args.cliprange)
                loss_sum += float(loss.detach())*args.grad_accum; clip_sum += float(meta.get('clip_fraction',torch.tensor(0.0))); acc+=1
                if acc==args.grad_accum or pos==len(order)-1:
                    gn=torch.nn.utils.clip_grad_norm_(policy.parameters(),1.0); opt.step(); opt.zero_grad(set_to_none=True); acc=0; updates+=1
